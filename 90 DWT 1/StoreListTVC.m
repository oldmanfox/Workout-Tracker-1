...
- (void)reload {
    ...
    [[DWT1IAPHelper sharedInstance] requestProductsWithCompletionHandler:^(BOOL success, NSArray *products) {
        if (success) {
            _products = products;
            [self.tableView reloadData];
        }
...
- (void)buyButtonTapped:(id)sender {
    ...
    [[DWT1IAPHelper sharedInstance] buyProduct:product];
}

- (void)restoreTapped:(id)sender {
    [[DWT1IAPHelper sharedInstance] restoreCompletedTransactions];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([[DWT1IAPHelper sharedInstance] productPurchased:product.productIdentifier]) {
