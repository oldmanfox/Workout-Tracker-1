...
+ (void)saveContext:(NSManagedObjectContext*)context {
    ...
    /*
     ...
     // This is the old code that doesn't cater to a context hierarchy
...
//    NSLog(@"dateStringValueTemp.length = %lu", (unsigned long)dateStringValueTemp.length);
//    NSLog(@"dateStringValueTemp = %@", dateStringValueTemp);
    
    if (dateStringValueTemp.length > 0) {
...
- (NSManagedObject*)copyUniqueObject:(NSManagedObject*)object
    ...
    NSString *dateStringValue = [NSDateFormatter localizedStringFromDate:[object valueForKey:uniqueAttribute]
                                                          dateStyle:NSDateFormatterShortStyle
                                                          timeStyle:NSDateFormatterLongStyle];
...
//    NSLog(@"uniqueAttributeValue = %@", uniqueAttributeValue);
//    NSLog(@"dateStringValue = %@", dateStringValue);
//    NSLog(@"dateStringValue.length = %lu", (unsigned long)dateStringValue.length);
    
    //NSLog(@"uniqueAttributeValue.length = %lu", (unsigned long)uniqueAttributeValue.length);
    ...
    if (dateStringValue.length  > 0) {
        ...
        // This was causing duplicate enteries when seeding to icloud.  Commented it out and now works good - Only migrates 1 unique object.
        // Update modified date as appropriate
        if ([[[copiedObject entity] attributesByName] objectForKey:@"date"]) {
            [copiedObject setValue:[NSDate date] forKey:@"date"];
        }
        return copiedObject;
    }
    return nil;
        ...
        for (NSManagedObject *sourceObject in sourceObjects) {
            ...
            if (sourceObject) {
                @autoreleasepool {
                    ...
                    [CoreDataImporter saveContext:targetContext];
                }
            }
        }
    }
